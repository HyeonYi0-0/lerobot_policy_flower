...
class FlowBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.adaLN_modulation = nn.Sequential(
            ...
            nn.Linear(lora_dim, 6 * dim),   # up-project to 6 AdaLN modulation signals
        )
...
class SharedAdaLNController(nn.Module):
    """Shared AdaLN controller for all DiT blocks — zero-initialized output projection."""
...
class ActionSpaceEmbedderParameter(nn.Module):
    """Embeds discrete action-space indices via learnable parameters."""
    def __init__(
        self,
        hidden_size,
        max_actions=11,
    ...
    def forward(self, action_indices):
        return self.mlp(self.action_embeddings[action_indices])
